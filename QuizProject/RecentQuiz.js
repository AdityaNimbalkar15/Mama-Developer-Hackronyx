    ...
    try {
      ...
      modBtn.onclick = () => {
        ...
        quizBtn.onclick = async () => {
          ...
          const qPrompt =
            `You are a course quiz generator.\nGenerate EXACTLY 5 multiple-choice questions (with 4 options each), based strictly on the technical content from these modules:\n` +
            `${sel.join('\n')}\n\nOnly 1 option should be correct. Wrap only the correct answer in <span class="answer"></span>.\n\n` +
            `Format:\nQ1. <question>\nA) <opt>\nB) <opt>\nC) <opt>\nD) <opt>`;
